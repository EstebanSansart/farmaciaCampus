...
public class MedicineBrandController : BaseApiController{
    ...
    public async Task<IEnumerable<MedicineBrandDto>> Get(){
       var records = await _UnitOfWork.Medicine_Brands.GetAllAsync();
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<MedicineBrandFullDto>> Get(int id){
       var record = await _UnitOfWork.Medicine_Brands.GetByIdAsync(id);
       if (record == null){
           return NotFound();
       }
       return _Mapper.Map<MedicineBrandFullDto>(record);
    }
    ...
    public async Task<ActionResult<Pager<MedicineBrandDto>>> Get11([FromQuery] Params conf){
       ...
       var records = await _UnitOfWork.Medicine_Brands.GetAllAsync(param);
    ...
    public async Task<ActionResult<MedicineBrandDto>> Post(MedicineBrandDto recordDto){
       var record = _Mapper.Map<Medicine_brand>(recordDto);
       _UnitOfWork.Medicine_Brands.Add(record);
    ...
    public async Task<ActionResult<MedicineBrandDto>> Put(int id, [FromBody]MedicineBrandDto recordDto){
       ...
       var record = _Mapper.Map<Medicine_brand>(recordDto);
       record.Id = id;
       _UnitOfWork.Medicine_Brands.Update(record);
       await _UnitOfWork.SaveChanges();
       return recordDto;
    }
    ...
    public async Task<IActionResult> Delete(int id){
       ...
       _UnitOfWork.Medicine_Brands.Remove(record);
       await _UnitOfWork.SaveChanges();
       return NoContent();
    }
}